...
namespace Webjet.DotNet.Common
{
    public class UrlUtils
    {
        ...
        public static void FailIfPhysicalPath(string path)
        {
            if (path != null && path.Length > 1)
            {
                if (path[1] == ':' || path.StartsWith(@"\\"))
                    throw new HttpException("Physical_path_not_allowed " + path);//MNF 19/8/2005 HttpRuntime.FormatResourceString("Physical_path_not_allowed", path));
            }
        }
        ...
        /* 
		 *  'System.Web.SessionState.SessionId' is inaccessible due to its protection level, so I commented it out
...
*/
        ...
        /// <returns></returns>
        public static string GetAbsoluteUrl(string path, HttpRequest httpRequest)
        {
            string retVal = path;

            if (!string.IsNullOrEmpty(path))
            {
                if (path.StartsWith("~") || IsRooted(path))
                {
                    ...
                    if (path.StartsWith("~"))
                    {
                        applicationUrl = path.Substring(1);
                    }

                    applicationUrl = (httpRequest.ApplicationPath + applicationUrl)
                        .Replace("//", "/");
                    ...
                    retVal = baseUrl + applicationUrl;
                }
                else
                {
                    if (httpRequest.IsSecureConnection)
                        retVal = path.Replace("http://", "https://");
                }
            }
