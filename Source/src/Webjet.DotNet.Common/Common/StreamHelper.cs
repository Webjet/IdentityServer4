   ...
   public static class StreamHelper
    {
		...
		/// You can use FindResourceAsString instead
		/// </summary>
        ...
        {   //NOTE: If resource is located in subfolder of C# project, path of subfolder should be specified (it is included as part of namespace)
            ...
            {// .Net 2 throws exception by itself
                throw new ApplicationException("Couldn't find embedded resource " + sName + "Consider to use FindResourceStream or FindResourceAsString instead");
            }
            return stream2;
        }

        public static T DeserializeResourceXml<T>(string resource, Assembly loadFrom)
        {
            using (var stream = StreamHelper.FindResourceStream(resource, loadFrom))
            using (var reader = new StreamReader(stream))
                return XmlSerializationHelper.DeserializeXml<T>(reader.ReadToEnd());
        }
        //
        /// <summary>
        ...
        /// <summary>
        /// Search through all embedded  resources, tolerant to missing subfolders in the name.
        /// Can be slightly less efficient than ResourceToString, that required fully specified name
        /// </summary>
        /// <param name="name"></param>
        ...
        public static string FindResourceFullName(string resName, Assembly assembly)
        {
            ...
            string message = String.Format("Could not find embedded resource {0} in assembly {1} ", resName, assembly.FullName);
